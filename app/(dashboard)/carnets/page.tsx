...
export default function CarnetsPage() {
  ...
  const [sociosFiltrados, setSociosFiltrados] = useState<Persona[]>([])
  ...
  const generarCarnetPDF = async (persona: Persona) => {
    // Usar la API route para generar el PDF
    ...
    <div className="space-y-6">
      ...
      <Card padding="md">
        <CardContent className="space-y-4">
          ...
          <div className="flex items-center gap-4 text-sm text-zinc-500 dark:text-zinc-400">
            <span>{sociosFiltrados.length} de {personas.filter(p => p.rol === 'socio').length} socios</span>
      ...
      <Card padding="md" className="bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800">
        ...
        <CardContent className="space-y-2 text-sm text-blue-800 dark:text-blue-300">
          <p>• El QR contiene el DNI del socio. Al escanear en el <strong>Kiosco</strong> (<kbd className="px-1.5 py-0.5 bg-white dark:bg-zinc-800 rounded border">/kiosco</kbd>) se registra entrada/salida.</p>
          <p>• Se genera un PDF por socio con: nombre, DNI, foto (placeholder), QR, plan, vigencia.</p>
          <p>• Imprimir en cartulina <strong>8.5 x 5.4 cm</strong> (tamaño tarjeta crédito) o A4 con 10 por hoja.</p>
          <p>• Socios sin DNI válido (7-8 dígitos) no generan QR — completar en ficha de persona.</p>
        </CardContent>
      </Card>
    </div>
  )
